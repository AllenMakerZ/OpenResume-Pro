...
const SortableSectionItem: React.FC<{ id: string; children: React.ReactNode; isDragDisabled?: boolean }> = ({ id, children, isDragDisabled }) => {
    const {
        attributes,
        listeners,
        setNodeRef,
...
const SortableRowItem: React.FC<{ id: string; children: React.ReactNode }> = ({ id, children }) => {
    const {
        attributes,
        listeners,
        setNodeRef,
    ...
    return (
        <div className="flex flex-col h-full bg-white border-r shadow-sm relative">
            ...
            <div className="px-4 border-b min-h-[68px] flex justify-between items-center flex-shrink-0">
                <h2 className="text-xl font-bold text-gray-800">OpenResume Pro</h2>
                <button
